...
export default function App() {
  ...
  return <SafeAreaView style={styles.safe}><StatusBar style="dark" /><View style={styles.app}><Header itemCount={itemCount} onHome={() => setScreen("home")} onCart={() => setScreen("cart")} /><ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>{error ? <View style={styles.error}><Text style={styles.errorText}>{error}</Text><Pressable onPress={() => setError("")}><Text style={styles.link}>Dismiss</Text></Pressable></View> : null}{screen === "home" && <Home products={featured} onShop={() => setScreen("shop")} onProduct={openProduct} />}{screen === "shop" && <Shop products={visibleProducts} collection={collection} setCollection={setCollection} onProduct={openProduct} />}{screen === "product" && selected && <ProductDetail product={selected} onBack={() => setScreen("shop")} onAdd={addToCart} />}{screen === "cart" && <Cart cart={cart} subtotal={subtotal} onBack={() => setScreen("shop")} onUpdate={(slug, quantity) => setCart((current) => current.map((item) => item.slug === slug ? { ...item, quantity } : item))} onRemove={(slug) => setCart((current) => current.filter((item) => item.slug !== slug))} token={token} user={user} />}{screen === "account" && <Account token={token} user={user} orders={orders} onSignedIn={async (nextToken, nextUser) => { setToken(nextToken); setUser(nextUser); await SecureStore.setItemAsync("hushwork_mobile_token", nextToken).catch(() => undefined); await SecureStore.setItemAsync("hushwork_mobile_user", JSON.stringify(nextUser)).catch(() => undefined); try { const result = await api("/orders", {}, nextToken); setOrders(result.orders as Order[]); } catch { setOrders([]); } }} onSignOut={signOut} />}</ScrollView><BottomNav screen={screen} itemCount={itemCount} onChange={setScreen} /></View></SafeAreaView>;
}
...
function Shop({ products, collection, setCollection, onProduct }: { products: Product[]; collection: string; setCollection: (value: string) => void; onProduct: (product: Product) => void }) {
