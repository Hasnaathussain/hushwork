...
export function AssistantDock() {
  ...
  const [messages, setMessages] = useState<Message[]>([{ role: "assistant", text: "Tell me what kind of evening you’re making. I’ll look through the current shelf." }]);
  ...
  return (
    <div className={`assistant-dock ${open ? "assistant-dock--open" : ""}`}>
      {open && <section className="assistant-panel" aria-label="HUSHWORK shopping guide"><div className="assistant-panel__header"><div><span className="assistant-orbit"><Sparkles size={14} /></span><div><p className="eyebrow">A little help</p><h2>Find your object</h2></div></div><button className="icon-button" type="button" onClick={() => setOpen(false)} aria-label="Close shopping guide"><X size={18} /></button></div><div className="assistant-messages">{messages.map((item, index) => <div className={`assistant-message assistant-message--${item.role}`} key={`${item.role}-${index}`}><p>{item.text}</p>{item.recommendations?.length ? <div className="assistant-recommendations">{item.recommendations.map((recommendation) => <Link href={`/product/${recommendation.slug}`} key={recommendation.slug} onClick={() => setOpen(false)}><span>{recommendation.name}</span><small>{recommendation.price} · {recommendation.reason}</small></Link>)}</div> : null}</div>)}{pending && <div className="assistant-message assistant-message--assistant"><p className="typing-dots"><i /><i /><i /></p></div>}</div><form className="assistant-form" onSubmit={ask}><input value={message} onChange={(event) => setMessage(event.target.value)} placeholder="I’m looking for…" aria-label="Ask the shopping guide" maxLength={600} /><button type="submit" aria-label="Ask the shopping guide" disabled={pending || !message.trim()}><ArrowUp size={17} /></button></form></section>}
      <button className="assistant-toggle" type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open}><span className="assistant-toggle__orb"><Sparkles size={17} /></span><span>{open ? "Close guide" : "Find my object"}</span></button>
    </div>
  );
}
